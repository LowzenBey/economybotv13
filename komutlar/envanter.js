...
exports.run = async (client, message, args) => {
  ...
  if(!db.kontrol(`para_${gösterilecek.id}`)) { var para = "0" } else { var para = db.bul(`para_${gösterilecek.id}`) }
  if(!db.kontrol(`iş_${gösterilecek.id}`)) { var iş = "Yok" } else { var iş = db.bul(`iş_${gösterilecek.id}`) }
if(!db.kontrol(`olta_${gösterilecek.id}`)) { var olta = "Yok" } else { var olta = db.bul(`olta_${gösterilecek.id}`) }
 if(!db.kontrol(`balta_${gösterilecek.id}`)) { var balta = "Yok" } else { var balta = db.bul(`balta_${gösterilecek.id}`) }
   if(!db.kontrol(`rol_${gösterilecek.id}`)) { var rol = "Yok" } else { var rol = db.bul(`rol_${gösterilecek.id}`) }
   if(!db.kontrol(`rozet_${gösterilecek.id}`)) { var rozet = "Yok" } else { var rozet = db.bul(`rozet_${gösterilecek.id}`) }
} else {
...
.setTitle(`${client.user.username} | Ekonomi`)
